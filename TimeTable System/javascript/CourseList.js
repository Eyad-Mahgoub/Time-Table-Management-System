    ...
    function ($scope, $http, $q) {
        ...
        if (session){
            var ajax = $http({ method: "GET", 
                            url:"URL removed Due to Privacy Concerns" 
                            });
        ...
        $scope.getSections = function(sub){
            sessionStorage.setItem("sub_code", sub.kod_subjek);
            $scope.section_list = sub.seksyen_list;
            $scope.table = false;
            $scope.details = true;
        }
        
        $scope.getDetails = function(sec){
            sessionStorage.setItem("sub_sec", sec);
        }
